...
export function PhoneFrame({
  ...
  let hasCustomBackground = Boolean(
    screenClassName
      ?.split(/\s+/)
      .some((utility) => utility.replace(/^!/, '').startsWith('bg-')),
  )
  ...
  return (
    <div className={clsx('relative aspect-366/729', className)} {...props}>
      ...
      <div
        className={clsx(
          'absolute top-[calc(23/729*100%)] left-[calc(23/366*100%)] grid h-[calc(686/729*100%)] w-[calc(318/366*100%)] transform grid-cols-1 overflow-hidden pt-[calc(23/318*100%)]',
          hasCustomBackground ? null : 'bg-gray-900',
          screenClassName,
        )}
      >
        {children}
