    ...
    <div className={clsx('flex flex-col', className)} {...props}>
      ...
      <div className="flex justify-between px-4 pt-4">
        <MenuIcon className="h-6 w-6 flex-none" />
          <Logo width={96} height={24} className="flex-none" />
...
AppScreen.Body = forwardRef<
  React.ElementRef<'div'>,
  { className?: string; children: React.ReactNode; flush?: boolean }
>(function AppScreenBody({ children, className, flush = false }, ref) {
  return (
    <div
      ref={ref}
      className={clsx(
        flush
          ? 'mt-0 flex-auto rounded-2xl bg-transparent'
          : 'mt-6 flex-auto rounded-t-2xl bg-white',
        className,
      )}
    >
      {children}
